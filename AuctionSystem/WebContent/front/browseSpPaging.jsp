 ...
 <c:forEach items="${splist }" var="sp">
			...
			<a href="javascript:;" onclick="javascript:product(${sp.id});">
			  <div class="col-sm-6 col-md-4" >
				...
				<c:if test="${pageCount!=0 }">
						<c:if test="${pageNum!=0}">
							<li><a href="<%=path %>/sp.Servlet?pageIndex=1&cmd=findSpByID&cid=${catalogid}">首页</a></li>
						</c:if>
						...
						<c:if test="${pageIndex>1}">
							<li><a href="<%=path %>/sp.Servlet?pageIndex=${pageIndex-1 }&cmd=findSpByID&cid=${catalogid}">
								上一页
							</a></li>
						</c:if>
						...
						<c:if test="${pageNum!=0}">
							<li><a href="<%=path %>/sp.Servlet?pageIndex=${pageNum}&cmd=findSpByID&cid=${catalogid}">
								尾页
							</a></li>
						</c:if>
						<c:if test="${pageNum==0}">
